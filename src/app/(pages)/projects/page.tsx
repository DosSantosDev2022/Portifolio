import { getAllProjects } from '@/services/hygraph/get-projects'
import { Projects, Pagination, ScrollAnimation } from '@/components/global'
...
export default async function ProjetcsPage({
	...
	const { project, totalCount } = await getAllProjects(page, first)
	...
	return (
		<div className='flex flex-col items-start justify-between gap-16 lg:gap-0'>
			<ScrollAnimation className='flex flex-col gap-2 px-3 py-8 lg:px-12 lg:py-16'>
				<h1 className={`${bebas.className} mt-10 text-8xl`}>
					Meus projetos
				</h1>
				<p className='text-muted text-lg'>
					Explore minha galeria de projetos! Descubra criações dinâmicas e
					envolventes, refletindo minha paixão e expertise fullstack.
					Prepare-se para se inspirar enquanto mergulha em soluções
					inovadoras!
				</p>
			</ScrollAnimation>
