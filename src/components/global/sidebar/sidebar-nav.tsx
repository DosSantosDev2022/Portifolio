...
const  SidebarNav = ({ links, title, isSocial = false }:SidebarNavProps) =>  {
  const pathname = usePathname();

  return (
    <div className='w-full'>
      {title && <div className='text-xs font-medium uppercase text-muted-foreground mb-4 px-4'>{title}</div>}
      <ul className='flex flex-col gap-2'>
        {links.map((link) => {
          ...
          return (
            <li key={link.href}>
              {isSocial ? (
                <Button variant={isActive ? 'secondary' : 'ghost'}  asChild  className='w-full justify-start gap-3'> 
